...
func main() {
	...
	fmt.Printf("The type of query parameters are %T\n", qparams) //Result will be url.Valuess which means it is a key b=value pair so we can use all the operations related to the key value pairs
	fmt.Println(qparams["coursename"])

	//printing all the query parameters
	//Here we don't need index so we are using the throwaway variable
